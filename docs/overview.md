...
`modernpackage` is a standalone package that, when invoked as `modernpackage <name>` or `mp <name>`, validates the package name, verifies required tools are on PATH, checks that the target directory does not already exist, and clones itself to a new directory and rewrites all occurrences of "modernpackage" to a normalized module name (converting hyphens and dots to underscores for import-safety). The result is a ready-to-build Python package with:
...
- **Preflight checks**: Before any git clone or filesystem mutation, the scaffolder runs three preflight checks in order: verify all required tools (`git`, `just`, `uv`) are on PATH, verify the target package directory does not already exist, and verify the template remote is reachable (via a `git ls-remote` probe with a timeout). These checks fail fast with actionable error messages before any clone subprocess is spawned or any directory is created
...
- **Preflight checks**: Before any scaffolding begins (before cloning or creating any directories), `init_new_package()` performs two preflight checks:
  1. **Tool verification** via `_verify_required_tools()`: verifies that `git`, `just`, and `uv` are all available on `PATH` via `shutil.which()`. If any tool is missing, scaffolding fails immediately with a clear, actionable error message naming all absent tools and providing an install pointer, without creating the target directory or running any subprocess. This fail-fast approach prevents confusing late failures (e.g., a missing `uv` surfacing only after a full clone and init as a cryptic validation failure).
  2. **Target directory check** via `_verify_target_directory_absent()`: verifies that the computed target package directory does not already exist (file or directory). If the path exists, scaffolding fails immediately with a clear, actionable error message suggesting the user choose a different package name or remove the existing directory. This prevents git clone from failing with a cryptic error message and ensures the user is aware of the conflict before any filesystem mutation.
