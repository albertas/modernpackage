...
- **Preflight checks**: Before any git clone or filesystem mutation, the scaffolder verifies that all required tools (`git`, `just`, `uv`) are on PATH and that the target package directory does not already exist. These preflight checks fail fast with actionable error messages before any subprocess is spawned or any directory is created
