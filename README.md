...
- `just publish` - automatically increment the patch version via `bump`, commit the version file to git with a descriptive message that includes the new version (e.g., "Bump version to 0.0.10"), push to the remote repository, then build and publish the package to pypi.org. The bumped version is included in both the GitLab repository and the PyPI release.
